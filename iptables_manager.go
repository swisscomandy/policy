...
type Chain interface {
	Setup(containerID, bridgeName string, ip net.IP, network *net.IPNet, externalIp net.IP, portRange string) error
...
func (mgr *IPTablesManager) ContainerSetup(containerID, bridgeName string, ip net.IP, network *net.IPNet, externalIp net.IP, portRange string) error {
	if err := mgr.ContainerTeardown(containerID); err != nil {
		return err
	}

	for index, chain := range mgr.chains {
		if err := chain.Setup(containerID, bridgeName, ip, network, externalIp, portRange); err != nil {
			for i := 0; i < index; i++ {
